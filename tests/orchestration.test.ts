...
beforeAll(() => {
  process.env.SUBAGENTS_DISABLE_CODEX = '1';
});

afterAll(() => {
  delete process.env.SUBAGENTS_DISABLE_CODEX;
});
...
describe('routing', () => {
  ...
  it('rewrites to orchestrator and creates todo', () => {
    const cwd = makeCwd();
    const routed = routeThroughOrchestrator({ agent: 'security', task: 'scan', cwd, mirror_repo: false });
...
describe('todo lifecycle', () => {
  it('records steps with outputs', async () => {
    const cwd = makeCwd();
    const routed = routeThroughOrchestrator({ agent: 'reviewer', task: 'check', cwd, mirror_repo: false });
...
describe('e2e multi-step', () => {
  it('tracks multiple steps', async () => {
    const cwd = makeCwd();
    const routed = routeThroughOrchestrator({ agent: 'orchestrator', task: 'plan', cwd, mirror_repo: false });
